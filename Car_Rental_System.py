...
def CarRent():
    try:
        ...
        def service(n,s):
            db = TinyDB("Path of your JSON database file.")
            user = Query()
        ...
        def afterPay(car, n, returnDay):    
            ...
            f1 = open("Path of your file containing cars that are currently rented", "a")
            f1.write(car+"\n")
            f1.close()

            db = TinyDB("Path of your JSON database file.")
            user = Query()
            db.update({"Date" : returnDay}, user.Aadhar == n)
            db.update({"Car" : car}, user.Aadhar == n)
        ...
        def returnCar(car, find, n):
            ...
            if car in find:
                ...
                if rec[0]["Car"] == car:
                    ...
                    f = open("Path of your file containing cars that are currently rented", "r")
                    fremove = f.readlines()
                    f = open("Path of your file containing cars that are currently rented", "w")
                    ...
                    db = TinyDB("Path of your JSON database file.")
                    user = Query()
                    db.update({"Car" : "No Car rented"}, user.Aadhar == n)
                    db.update({"Date" : "No due date"}, user.Aadhar == n)
                    input(Fore.CYAN + "\nPress Enter to continue.\n" + Fore.RESET)
                    return ShowServices(n)
        ...
        def userDetails(n):
            ...
            print(Fore.GREEN + "Aadhar Number:" + Fore.RESET, n)
            ...
            while k != 1:
                try:
                    ...
                    if (len(str(Ano)) > 12) or (len(str(Ano)) < 12):
                        ...
                        print(Fore.RED + "\nCustomer already exist" + Fore.RESET)
